<script lang="ts">
	...
	let compoundedOpportunities = $derived.by(() => {
		// eslint-disable-next-line svelte/prefer-svelte-reactivity -- local map rebuilt each derivation, not reactive state
	...
	function getMargin(item: DashboardItem): number | null {
		...
		if (item.itemId == 13190) raw = raw - 0.1 * item.highPrice; // oldchool bonds require an additional 10% fee
		return f.showTax ? raw - calcTax(item.highPrice) : raw;
	}
	...
	let filtered = $derived.by(() => {
		...
		for (const col of columns) {
			...
			const cf = f.columnFilters[col.key];
			if (!cf) continue;
			const minVal = parseNumeric(cf.min);
			const maxVal = parseNumeric(cf.max);
		...
		return items.toSorted((a, b) => {
			...
			if (f.sortKey === 'name') {
				...
				av = getColumnValue(a, f.sortKey);
				bv = getColumnValue(b, f.sortKey);
			}
	...
	{#if compoundedOpportunities.length > 0 || soloSignals.length > 0}
		<div class="mb-6">
			...
			<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
				...
				{#each compoundedOpportunities.slice(0, 6) as item (item.itemId)}
					{@const mPct =
					...
					>
						<div class="flex items-center justify-between">
							...
							<div class="flex flex-wrap gap-1">
								{#each item.signals as signal (signal.signalType)}
									<!-- eslint-disable-next-line svelte/no-navigation-without-resolve -->
				...
				{#each soloSignals.slice(0, Math.max(0, 6 - compoundedOpportunities.length)) as signal (signal.itemId + ':' + signal.signalType)}
